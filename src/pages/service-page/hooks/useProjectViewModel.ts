...
import { NewRecordProject, ProjectTableDataType } from '../type'

export default function useProjectViewModel() {
  const { message } = AntApp.useApp()
  const table = useTable<ProjectTableDataType>([])

  const service = useAPIService<Project>(ProjectAPI)

  const [openModalCreate, setOpenModalCreate] = useState<boolean>(false)
  const [recorded, setRecorded] = useState<Project>({ id: 0 })
  const [newRecord, setNewRecord] = useState<NewRecordProject | null>(null)
  ...
  const handleCreate = async (itemNew: NewRecordProject) => {
    try {
      await service.createItemSync({ ...itemNew } as Project, table.setLoading, (res) => {
        if (!res.success) throw new Error(define('create_failed'))
        const newItem = res.data as Project
  ...
  const handleUpdate = async (record: ProjectTableDataType) => {
    try {
      ...
      await service.updateItemByPkSync(record.id!, { ...newRecord }, table.setLoading, (res) => {
        ...
        const updatedItem = res.data as Project
  ...
  const handleDelete = async (record: ProjectTableDataType) => {
    try {
      table.setLoading?.(true)
  ...
  const handleDraggableEnd = async (newArr: ProjectTableDataType[]) => {
    try {
