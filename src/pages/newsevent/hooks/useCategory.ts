...
import { AttachmentTableDataType } from '../type'
...
export default function useCategory(table: UseTableProps<AttachmentTableDataType>) {
    ...
    setDataSource(
      items.map((item) => {
        return {
          key: `${item.id}`,
          ...item
        } as AttachmentTableDataType
      })
    )
  }

  const handleSaveClick = async (record: AttachmentTableDataType) => {
