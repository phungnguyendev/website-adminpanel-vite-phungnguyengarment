...
import { NewRecordPrize, PrizeTableDataType } from '../type'

export default function usePrizeViewModel() {
  const { message } = AntApp.useApp()
  const table = useTable<PrizeTableDataType>([])

  const service = useAPIService<Prize>(PrizeAPI)

  const [openModalCreate, setOpenModalCreate] = useState<boolean>(false)
  const [recorded, setRecorded] = useState<Prize>({ id: 0 })
  const [newRecord, setNewRecord] = useState<NewRecordPrize | null>(null)
  ...
  const handleCreate = async (itemNew: NewRecordPrize) => {
    try {
      await service.createItemSync({ ...itemNew } as Prize, table.setLoading, (res) => {
        if (!res.success) throw new Error(define('create_failed'))
        const newItem = res.data as Prize
  ...
  const handleUpdate = async (record: PrizeTableDataType) => {
    try {
      ...
      await service.updateItemByPkSync(record.id!, { ...newRecord }, table.setLoading, (res) => {
        ...
        const updatedItem = res.data as Prize
  ...
  const handleDelete = async (record: PrizeTableDataType) => {
    try {
      table.setLoading?.(true)
  ...
  const handleDraggableEnd = async (newArr: PrizeTableDataType[]) => {
    try {
