...
const PrizeTable: React.FC = () => {
  ...
  const columns = {
    ...
    actionCol: (record: PrizeTableDataType) => {
      return (
        <SkyTableActionRow
          record={record}
  ...
  const actionCol: ColumnType<PrizeTableDataType> = {
    title: 'Operation',
    width: '0.001%',
    render: (_value: any, record: PrizeTableDataType) => {
            ...
            <>
              <Flex className='w-full'>
                <Typography.Text type='secondary' className='text-xl font-semibold'>
                  Prizes ({viewModel.table.dataSource.length})
                </Typography.Text>
              </Flex>
            </>
          }
