...
export default {
  createItem: async (item: Product, accessToken: string): Promise<ResponseDataType | undefined> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItem: async (id: number, itemToUpdate: Product, accessToken: string): Promise<ResponseDataType | undefined> => {
    return client
