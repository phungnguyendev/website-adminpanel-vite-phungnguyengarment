...
export default {
  createItem: async (item: Partner, accessToken: string): Promise<ResponseDataType | undefined> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItem: async (id: number, itemToUpdate: Partner, accessToken: string): Promise<ResponseDataType | undefined> => {
    return client
