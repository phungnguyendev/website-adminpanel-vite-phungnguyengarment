...
export default {
  createItem: async (item: Post, accessToken: string): Promise<ResponseDataType | undefined> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItem: async (id: number, itemToUpdate: Post, accessToken: string): Promise<ResponseDataType | undefined> => {
    return client
