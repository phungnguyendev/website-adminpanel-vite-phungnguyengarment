...
export default {
  createItem: async (item: ProductCategory, accessToken: string): Promise<ResponseDataType | undefined> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItem: async (id: number, item: ProductCategory, accessToken: string): Promise<ResponseDataType | undefined> => {
    return client
