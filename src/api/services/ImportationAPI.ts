...
export default {
  createNewItem: async (item: Importation, accessToken: string): Promise<ResponseDataType | undefined> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItemByPk: async (id: number, item: Importation, accessToken: string): Promise<ResponseDataType | undefined> => {
    return client
      .put(
        `${NAMESPACE}/${id}`,
        {
