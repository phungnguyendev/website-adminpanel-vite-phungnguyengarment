...
const NAMESPACE = 'industry-sectors'

export default {
  createNewItem: async (item: IndustrySector, accessToken: string): Promise<ResponseDataType | undefined> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateList: async (itemsToUpdate: IndustrySector[]): Promise<ResponseDataType | undefined> => {
    return client
